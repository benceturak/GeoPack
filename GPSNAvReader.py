...
from normalizeTime import normalizeTime
import math
...
class GPSNavReader(object):
    ...
    def getValidEph(self, prn, epoch):
        ...
        for nav in self.navigationDatas[prn]:
            ...
            minEq = True
            maxEq = True
            for i in np.append(frame, [nav[0:6]], axis=0).T:
                minEq = i[0] == i[2]
                if not minEq:
                    if i[0] <= i[2]:
                        minCond = True
    ...
    def getSatPos(self, prn, epoch):
        ...
        t = DOW*86400 + epoch[3]*3600 + epoch[4]*60 + epoch[5]
...
if __name__ == "__main__":
    ...
    #print(reader.getValidEph('G08', np.array([2019, 4, 2, 7, 48, 52])))
    print(reader.getSatPos('G08', np.array([2019, 4, 2, 7, 48, 52])))
    #print(reader.beta)
